...
2. 交易類別 type 判斷：
   ...
   - 「存款息」或銀行存款利息（如「存款息」2 元）-> type 設為 "cash_in"，不需 symbol，備註寫「銀行存款利息」，member 設為 "yun"
   - 現金類（cash_in / cash_out）的 member 一律填 "yun" 或 "bro"，不要用 "both"；若無法判斷歸屬（如銀行利息、不知名小額收支），一律歸 "yun"
3. 證券 App 特殊排版與欄位解析：
