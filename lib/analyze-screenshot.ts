 ...
 * 與框架無關：本地 Express (server.ts) 與 Vercel Serverless Function
 * (api/analyze-screenshot.ts) 共用同一份。金鑰只在 server 端讀取。
 */
...
const RESPONSE_SCHEMA = {
  ...
  properties: {
    isBatch: {
      type: Type.BOOLEAN,
      description: 'Whether multiple transactions or bank entries are detected in the screenshot',
    },
    transactions: {
      type: Type.ARRAY,
 ...
 * @param imageDataUrl data URL 或純 base64 字串
 * @throws 若未設定金鑰、或 AI 回傳空/解析失敗
 */
